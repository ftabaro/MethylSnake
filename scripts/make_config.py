...
def make_parser():
    ...
    parser.add_argument("--config-path", required=True, help="Desired path for the config file")
    parser.add_argument("--wd", required=True, help="Working directory")
    parser.add_argument("--genome-path", required=True, help="Path to a fasta file with genome sequence")
    parser.add_argument("--bismark-index-path", required=True, help="Path to Bismark index files (only index name required: /path/to/index/folder/hg38*)")
    parser.add_argument("--sample-sheet", required=True, help="Path to csv file holding samples information")

    parser.add_argument("--annotation-file", required=False, help="Path to a gzipped GTF file.")
#    parser.add_argument("--ensembl-root", required=False)
#    parser.add_argument("--ensembl-version", required=False)

    # parser.add_argument("--environments-folder", default="../environments", help="")
    parser.add_argument("--singularity-container", default="/home/ft413468/containers/methylkit.sif", help="Path to a singularity container with all necessary tools installed.")

    parser.add_argument("--tmp-folder", default="tmp", help="Path to a temporary folder (possibly fast storage)")
    parser.add_argument("--log-folder", default="~/var/log", help="Path to a log folder")
    parser.add_argument("--reads-folder", default="reads", help="Path to a folder with reads to be processed")
    parser.add_argument("--trimmed-folder", default="trimmed", help="Path to folder to write trimmed reads in")
    ...
    parser.add_argument("--dmr-window-size", default=500, help="Window size for tiled differential methylation analysis")
    ...
    parser.add_argument("--fastq-extension", default=".fq.gz", help="File extension of reads fastq files")

    parser.add_argument("--genome-version", default="hg38", help="Label for genome version used in the analysis")
